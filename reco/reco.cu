...
int forward_project_host(sm_part *sm, uint smstart, uint smend, uint *pet, float* vol,int ring, float* K, int dzcut, float valcut)
{	
	uint smpos = smstart;
	//uint count = 0;
	while (smpos < smend) {
		for(int phi=0;phi<cryNum;phi++){  // for loop over threads
			...
			if(tl.z1==0 && tl.z2==0 && tl.c2 <= tl.c1) continue;  // skip unused case
...
int backward_project_host(sm_part* sm, uint smstart, uint smend, uint *pet, int ring, float* K, float* M, int dzcut, float valcut)
{
	uint smpos = smstart;
	while (smpos < smend) {
		for(int phi=0;phi<cryNum;phi++){  // for loop over threads
			...
			if(tl.z1==0 && tl.z2==0 && tl.c2 <= tl.c1) continue; // skip unused case
			int dc = c2_to_dc2(tl.c1,tl.c2);         // sm has actual c2 not delta		
			int dz = tl.z1+tl.z2;  // net delta z
			float val= sm[smpos].val;
			if(dz > dzcut || val < valcut) break;
			...
			for (int zs1 = 0; zs1 < zNum-dz; zs1++) {  // zs1 is sliding posn of lh end of lor 
				if(pet[lor_index]>0){ 	
					//if(K[lor_index == 0.0f]) printf("zero K  found index %u\n",lor_index);
...
int rescale_host(r_Ptr<float> vol, cr_Ptr<float> M, cr_Ptr<float> norm)
{
	for(int id=0;id<zNum*radNum;id++){
		float scale = 1.0f/norm[id];
		for(int phi=0;phi<cryNum;phi++) vol[id*cryNum+phi] *= M[id*cryNum+phi]*scale;
	}
	return 0;
...
// assumes threads = cryNum i.e. 400 so that one thread blocks process all phis for fixed sm value
__global__  void forward_project(cr_Ptr<sm_part> sm, uint smstart, uint smend, cr_Ptr<uint> pet, cr_Ptr<float> vol,int ring, r_Ptr<float> K, int dzcut, float valcut)
{
	int phi = threadIdx.x;
	...
	while (smpos < smend) {
		...
		uint vol_index = (ring*zNum + tl.z1)*cryNum + phi;      // z+z1 here as voxel index
		for (int zs1 = 0; zs1 < zNum-dz; zs1++) {  // zs1 is sliding posn of lh end of lor
			//uint lor_index = (zdz_slice(zs1) + dz)*cryCdCNum + dc*cryNum + tl.c1; // just zs1 here as z1dz2 format lor index 
			if(pet[lor_index]>0){  
				float element = vol[vol_index] * val; 		
...
// assumes threads = cryNum i.e. 400 so that one thread blocks process all phis for fixed sm value
__global__  void backward_project(cr_Ptr<sm_part> sm, uint smstart, uint smend, cr_Ptr<uint> pet, int ring, cr_Ptr<float> K, r_Ptr<float> M, int dzcut, float valcut)
{
	int phi = threadIdx.x;
	uint smpos = smstart+blockIdx.x;
	...
	while (smpos < smend) {
		...
		int dz = tl.z1+tl.z2;  // net delta z
		float val= sm[smpos].val;
		if(tl.z1==0 && tl.z2==0 && tl.c2 <= tl.c1) val *= 0.5f;  // try this
		if(dz > dzcut || val < valcut) {smpos += gridDim.x; continue; }
		...
		for (int zs1 = 0; zs1 < zNum-dz; zs1++) {  // zs1 is sliding posn of lh end of lor
			//uint lor_index = (zdz_slice(zs1) + dz)*cryCdCNum + dc*cryNum + tl.c1; // just zs1 here as z1dz2 format lor index 
			if(pet[lor_index]>0){ 				
				float element = val * pet[lor_index] / K[lor_index]; 
				atomicAdd(&M[vol_index],element);                     // bwd 6445.918 ms
				//if(threadIdx.x> 512) printf("dummy %f\n",element);  // bwd 2593.449 ms
			}
			lor_index += cryCdCNum;  // for zs1++
			vol_index += cryNum;     // for zs1++
		}
...
int normalise_sm(thrustHvec<sm_part> &sm,thrustHvec<float> &norm,thrustHvec<uint> &smhits,thrustHvec<uint> &smstart, int dzcut, float valcut)
{
	...
	for(int r = 0;r<radNum;r++){
		...
		for(uint k=sm_start;k<sm_start+smnum;k++){
			...
			float val = sm[k].val;
			int dz = tl.z2+tl.z1;  // was - changed 9/7/19
			if(dz > dzcut || val < valcut) continue;
			for (int z = tl.z1; z < zNum - tl.z2; z++)  norm[r*zNum+z] += val;	  //vertex posn here
		}
	}
	printf("normalization done for %d rings and %d slices\n",radNum,zNum);	
	for(uint i=0;i<norm_size;i++) norm[i] /= 1.0e+10;  // assume 10^10 generations per voxel
...
{
	if(argc < 2){
		printf("usage reco <pet file (phantom)> <result file> <sm file> <sm nhits file> <iterations> [ dzcut|63] valcut|0 usehost|0] blocks|512\n");
		return 0;
	}
	...
	int dzcut = 63; if(argc> 6) dzcut = atoi(argv[6]);
	float valcut = 0.0f; if(argc> 7) valcut = atof(argv[7]);
	int usehost = 0; if(argc> 8) usehost = atoi(argv[8]);
	// set up system matix
	char name[256];
	...
	int threads = cryNum;
	int blocks = 50000*36; if(argc> 9) blocks = atoi(argv[9])*36;  // 36 = number of sm units on gpu
	printf("using blocks = %d, thread = %d\n",blocks,threads);
	if(argc> 8) usehost = atoi(argv[8]);
	...
	//uint vbuf_size = blocks*cryNum*zNum;
	...
	//thrustHvec<float>     vbuf(vbuf_size); // voxel normaliztions depend on ring and z
	//thrustDvec<float> dev_vbuf(vbuf_size);

	cx::MYTimer ntim;
	ntim.start();
	normalise_sm(sm,norm,nhits,nhits_start,dzcut,valcut);
	...
	if(usehost)for(int iter = 0;iter< niter;iter++){
		...
		tim1.reset();
		for (int r = 0; r < radNum; r++) {
			forward_project_host(sm.data(), nhits_start[r], nhits_start[r]+nhits[r+2], pet.data(),vol.data(), r, K.data(),dzcut,valcut );
		}
		tim1.add();
		tim2.reset();
		for (int r = 0; r < radNum; r++) {
			backward_project_host(sm.data(), nhits_start[r], nhits_start[r]+nhits[r+2], pet.data(), r, K.data(), M.data(),dzcut,valcut );
		}
		tim2.add();
		tim3.reset();
		rescale_host(vol.data(),  M.data() , norm.data());
		tim3.add();
		...
		tim1.reset();
		for (int r = 0; r < radNum; r++) {
			forward_project<<<blocks, threads>>>(dev_sm.data().get(), nhits_start[r], nhits_start[r]+nhits[r+2], dev_pet.data().get(), dev_vol.data().get(), r, dev_K.data().get(),dzcut,valcut );
		}
		checkCudaErrors(cudaDeviceSynchronize());
		tim1.add();
		tim2.reset();
		for (int r = 0; r < radNum; r++) {
			backward_project<<<blocks, threads>>>(dev_sm.data().get(), nhits_start[r], nhits_start[r]+nhits[r+2], dev_pet.data().get(), r, dev_K.data().get(), dev_M.data().get(),dzcut,valcut );
		}
		checkCudaErrors(cudaDeviceSynchronize());
		tim2.add();
		tim3.reset();
