...
__global__  void forward_project(cr_Ptr<sm_part> sm, uint smstart, uint smend, cr_Ptr<uint> pet, cr_Ptr<float> vol,int ring, r_Ptr<float> K)
{
	...
	while (smpos < smend) {
		...
		for (int zs1 = 0; zs1 < zNum-tsum; zs1++) {  // zs1 is sliding posn of lh end of lor
			...
			////uint lor_index = (zdz_slice(tsum) + zs1)*cryCdCNum + dc*cryNum + tl.c1; // z2 not z
			//uint lor_index = dc*cryNum*detZdZNum + (zdz_slice(tsum) + zs1)*cryNum + tl.c1; // z2 not z
			if(pet[lor_index]>0){  // most lors zero for nice phantoms
				//uint vol_index = (ring*zNum + zs1+tl.z1)*cryNum + phi;      // z+z1 here as voxel index
...
__global__  void backward_project(cr_Ptr<sm_part> sm, uint smstart, uint smend, cr_Ptr<uint> pet, int ring, cr_Ptr<float> K, r_Ptr<float> M)
{
	...
	while (smpos < smend) {
		...
		for (int zs1 = 0; zs1 < zNum-tsum; zs1++) {  // zs1 is sliding posn of lh end of lor
			...
			if(pet[lor_index]>0){  // most lors zero for nice phantoms
				int vol_index = (ring*zNum + zs1+tl.z1)*cryNum + phi;       // z+z1 here as voxel index
