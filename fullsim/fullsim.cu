...
__device__ int ray_to_cyl(Ray &g,Lor &l,float length)  //15/6/19 added length argument  
{
	...
	if(z1 >= 0.0f && z1 < length && z2 >= 0.0f && z2 < length && abs(z2-z1) < detLen ){ // same zdiff short and long detectors
		...
		// enforcing special treatment of in plane slices, namely c1< c2, just causes pain in subsequent code
		//else if(l.z1==l.z2 && l.c1 > l.c2) cx::swap(l.c1,l.c2); //NB special case here - take care alsewhere
		return 1;
	}
...
__global__ void find_spot(r_Ptr<uint> map,r_Ptr<uint> spot,Roi vox)
{
	...
	float z_id = blockIdx.x;  // left hand z, i.e z1
...
// NB changes for book zms2 cosmetic, 
// breaks old code zms1 not 63-zms1, uint *ngood
template <typename S> __global__ void cylgen_book2(r_Ptr<uint> map,r_Ptr<uint> ngood, Roi roi,S *states,uint tries)
{
	int id = threadIdx.x + blockIdx.x*blockDim.x;
	S state = states[id];
	Ray g;
	...
	for(uint k=0;k<tries;k++){
		...
		if(ray_to_cyl(g,lor,detLongLen)){
			good++;
			uint zms2 = max(0,lor.z2-zNum+1); 
			uint zms1 = max(0,zNum-lor.z1-1); 
			uint index = (zms1*cryNum+lor.c1)*mapSlice+(zms2*cryNum+lor.c2);
...
template <typename S> __global__ void cylhits(r_Ptr<uint> hits,r_Ptr<uint> map,r_Ptr<ushort> vfill,r_Ptr<double> ngood, Roi roi,S *states,uint tries)
{
	...
	for(uint k=0;k<tries;k++){
		...
		if(0){  // This to save generated decay points - for debug only
			int3 p;
		...
		if(ray_to_cyl(g,lor,detLongLen)){
			...
			//}
			if(0){  // more debug - display distibutions for detecxted decays
			...
			if(1){  // z1 <= 63 and z2 >= 63 in long scanner
				if(lor.z2<zNum-1) printf("unexpected z2 = %d\n",lor.z2);
				uint lor2sub = max(0,lor.z2-zNum+1);  // this is zsm2
				//if(lor.z1 <1) printf("unexpected z1 = %d\n",lor.z1);  // these for 128 long sanner
				//uint lor1sub = max(0,lor.z1-1);
				if(lor.z1 <0) printf("unexpected z1 = %d\n",lor.z1);
				uint lor1sub = max(0,lor.z1);  // NB this is 63 - xsm1 (i.e displacement from LH end not decay point)
	...
	for(uint k=0;k<tries;k++){
		...
		if(ray_to_cyl(g,lor,detLen)){
			...
			if(cdiff >= cryDiffMin && cdiff <=cryDiffMax){
				...
				//uint zdz =    zdz_slice(lor.z1) + lor.t2-lor.z1;   // zdz_slice(zs1) + dz in reco
...
int main(int argc,char *argv[])
{
	if(argc < 2){
		...
		printf(" mode = 1 <r1> <r2> [savelors|1] generate smat files radius from <vr1>  to <vr2>-1 \n");
